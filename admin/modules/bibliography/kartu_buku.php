 ...
 */
 ...
 /* This source modified by Muh Tarom (stuqly@gmail.com) on Friday, 21 December 2012*/
...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	$chunked_book_card_arrays = array_chunk($item_data_array, $bookcard_items_per_row);
	// create html ouput
	$html_str .= '<!DOCTYPE html>'."\n";
	$html_str .= '<!-- Contributed(@)2012 Muh Tarom (stuqly@gmail.com) -->'."\n";
	$html_str .= '<html lang="en">'."\n";
	$html_str .= '<head>'."\n";
	$html_str .= '<title>Hasil Cetak Kartu Buku</title>'."\n";
	...
	$html_str .= 'body { background: #ffffff; color: #000000; font-family: '.$bookcard_fonts.'; font-size: 9pt; margin: 1cm; padding: 0px; }'."\n";
	$html_str .= '.bookcard { min-height: '.$bookcard_height.'; width: '.$bookcard_width.'; margin: 0.5cm; }'."\n";
	...
	foreach ($chunked_book_card_arrays as $bookcard_rows) {
		$html_str .= '<tr>'."\n";
		foreach ($bookcard_rows as $bookcard) {
			$html_str .= '<td valign="top">'."\n";
				$html_str .= '<div class="bookcard">'."\n";
				if ($bookcard_include_header_text) { $html_str .= '<div class="libname">'.($bookcard_header_text?$bookcard_header_text:$sysconf['library_name']).'<br />'.($bookcard_address_text).'</div>'."\n";}
				...
				$html_str .= '<td valign="top" style="margin: 0;">'.$bookcard[3].'</td>'."\n";
				$html_str .= '</tr>'."\n";
				$html_str .= '<tr>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; width: 80px;">Judul</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; width: 6px;">:</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0;">';
				if ($bookcard_cut_title) {
					$html_str .= substr($bookcard[6], 0, $bookcard_cut_title).'..';
				} else { $html_str .= $bookcard[6]; }
				$html_str .= '</td>'."\n";
				$html_str .= '</tr>'."\n";
				$html_str .= '<tr>'."\n";
				...
				$author_q = $dbs->query('SELECT b.biblio_id, b.title, a.author_name FROM biblio AS b
					...
					WHERE b.biblio_id='.$bookcard[1]);
				$authors = '';
				while ($author_d = $author_q->fetch_row()) {
					$bookcard[6] = $author_d[1];
					$authors .= $author_d[2].' - ';
				}
				$authors = substr_replace($authors, '', -2);
				if ($bookcard_cut_authors) {
					$html_str .= substr($authors, 0, $bookcard_cut_authors).'..';
				...
				$html_str .= '<td valign="top" style="margin: 0; width: 6px;">:</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0;">'.$bookcard[4].'</td>'."\n";
				$html_str .= '</tr>'."\n";
				$html_str .= '</table>'."\n";
				$rows = $bookcard_number_row;
				$html_str .= '<table style="margin: 5px 0px 0px 0px; padding: 0; width: 8.6cm;" cellspacing="0" cellpadding="0" border="1px">'."\n";
				$html_str .= '<tr>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.7cm;">Tgl</br>Pinjam</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 3.0cm;">Nama Peminjam</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.6cm;">Prf</br>Peminjam</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.7cm;">Tgl</br>Kembali</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 2.0cm;">Prf</br>Petugas</td>'."\n";
				$html_str .= '</tr>'."\n";
				for($tr=1;$tr<=$rows;$tr++){
				$html_str .= '<tr>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.7cm;">&nbsp;</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 3.0cm;">&nbsp;</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.6cm;">&nbsp;</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 1.7cm;">&nbsp;</td>'."\n";
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 2.0cm;">&nbsp;</td>'."\n";
				$html_str .= '</tr>'."\n";}
				$html_str .= '</table>'."\n";
				$html_str .= '</div>'."\n";
				$html_str .= '</td>'."\n";
	...
	if ($file_write) {
		...
		echo '<script type="text/javascript">top.$.colorbox({href: "'.SWB.FLS.'/'.$print_file_name.'", iframe: true, width: 800, height: 500, title: "Hasil Cetak Kartu Buku"})</script>';
...
<div class="menuBoxInner printIcon">
	<div class="per_title">
    <h2><?php echo __('Kartu Buku Printing'); ?></h2>
  </div>
	<div class="sub_section">
    <div class="btn-group">
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/kartu_buku.php?action=clear" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/kartu_buku.php?action=print" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Cetak Kartu Buku'); ?></a>
	</div>
    <form name="search" action="<?php echo MWB; ?>bibliography/kartu_buku.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
