...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  $html_str .= '.leftSide { display: table-cell; float: left; width: 33%; text-align: center; margin-top: 20px; margin-left: 4px;transform:rotate(-0deg); }'."\n";
  $html_str .= '.rightSide { float: right; width: 64%; height: '.$sysconf['print']['label']['box_height'].'cm; border-left: '.$sysconf['print']['barcode']['barcode_border_size'].'px solid #000000; font-size: '.$sysconf['print']['label']['font_size'].'pt; text-align: center; }'."\n";
  $html_str .= '.barcode { display: table-cell; text-align: center; vertical-align: middle; }'."\n";
  $html_str .= '.callnumb { font-size: 13pt; font-weight: bold; float: center; display: table-cell; text-align: center; margin-top: 4px; width: 5.5cm; }'."\n";
  $html_str .= '.labelHeaderStyle { border-bottom: '.$sysconf['print']['barcode']['barcode_border_size'].'px; background-color: #CCCCCC; font-weight: bold; padding: 5px; margin-bottom: 5px; border-bottom: 1px; text-align: center; }'."\n";
  $html_str .= 'img { -moz-transform: rotate(0deg); -ms-transform:rotate(0deg); -o-transform:rotate(0deg); -webkit-transform:rotate(0deg); width: '.$sysconf['print']['barcode']['barcode_scale'].'%; }'."\n";
  $html_str .= '</style>'."\n";
  $html_str .= '</head>'."\n";
  $html_str .= '<body>'."\n";
  ...
  if ($file_write) {
    ...
    echo '<script type="text/javascript">top.$.colorbox({href: "'.SWB.FLS.'/'.$print_file_name.'", iframe: true, width: 850, height: 500, title: "'.__('Label Barcodes modified by CV. Prime-In').'"})</script>';
...
<div class="menuBoxInner printIcon">
  ...
  <div class="sub_section">
      ...
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/label_barcode_generator.php?action=print" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Labels for Selected Data');?></a>
